...
Plugin event sequence numbers are positive, contiguous, and persistent across process restarts. The telemetry stream ID survives those restarts but changes when the plugin's private data is deliberately removed, allowing a later installation to start again at sequence one. The plugin retains an event until the Agent advances past its sequence. If the requested cursor is older than retained data, the plugin returns a data-loss error rather than skipping records. Access event IDs are stable within that stream; the Agent adds the stream ID to the standard event so the center can deduplicate retries without confusing a later installation. Source IPs are canonical strings and events never contain credentials or complete client configuration.
